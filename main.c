...
///////below this are new includes////////
#include "gpio.h"
#define MXC_GPIO_PORT_OUT MXC_GPIO1
#define MXC_GPIO_PIN_OUT MXC_GPIO_PIN_6
/*  
    //in int main() function, add the following line to
    //set up gpio pin
...
const char classes[CNN_NUM_OUTPUTS][10] = { "clean", "unclean" };
...
{   
    ...
    //MXC_Delay(300000);
    printf("\n\nclean_unclean Feather Demo\n");
#elif defined(BOARD_EVKIT_V1)

    printf("\n\nclean_unclean Evkit Demo\n");
#else
    printf("\n\nclean_unclean CAM02 Demo\n");
#endif
    ...
    TFT_Print(buff, 25, 90, font_1, snprintf(buff, sizeof(buff), "Clean_Unclean_Detector"));
    ...
    while (1) {
...
#ifdef TFT_ENABLE
        // int threshold = 80; // 80% threshold for clean vs unclean decision
        ...
        // Class = clean
            ...
            LED_Off(LED1);
            LED_Off(LED2);
            // Set GPIO pin to LOW if clean detected
            // printf("clean\n");
            MXC_GPIO_OutClr(gpio_out.port, gpio_out.mask); // set pin low
        ...
        // Class = unclean
            ...
            LED_On(LED1);
            LED_Off(LED2);
            // Set GPIO pin to HIGH if unclean detected
            // printf("unclean\n");
            MXC_GPIO_OutSet(gpio_out.port, gpio_out.mask); // set pin high
